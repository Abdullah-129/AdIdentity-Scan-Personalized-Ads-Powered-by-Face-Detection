...
while cv2.waitKey(1) < 0:
    ...
    if len(faceBoxes) == 0:
        if current_video is not None and not video_not_found:
        ...
        for faceBox in faceBoxes:
            ...
            if age != current_age or gender != current_gender or video_not_found:
                current_age = age
                current_gender = gender
                video_not_found = False

                age_folder_path = os.path.join(folder_path, age)
                ads = [f"{age_folder_path}/{i}.mp4" for i in range(1, 11)]
                random_video = random.choice(ads)

                if os.path.exists(random_video):
                    if current_video is not None:
                        current_video.release()

                    current_video = cv2.VideoCapture(random_video)
                    print(f"Playing ad: {random_video}")
                else:
                    print(f"Video file not found: {random_video}")
                    current_video = None
                    video_not_found = True
